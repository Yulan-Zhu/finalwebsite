...
var map = L.map('map')
   ...
   .setView([22.2854, 114.1607], 11);

 layerGroup = L.layerGroup().addTo(map);

 var items = [];
 var airtable_read_endpoint = "https://api.airtable.com/v0/appCNIFAZSVHE8FTp/hk?api_key=keyEKVb9dgJuIAFSg";
 var data = [];
...
$(document).ready(function(){
  ...
  $("button#get_data1").click(function() {
      ...
      var airtable_read_endpoint = "https://api.airtable.com/v0/appCNIFAZSVHE8FTp/Table%201?api_key=keyEKVb9dgJuIAFSg";
      $.getJSON(airtable_read_endpoint, function(result) {
            $.each(result.records, function(key,value) {
                items = [];
   ...
   $("button#get_data2").click(function() {
        ...
        var i = 0;
        $('.table-responsive').show();
        var airtable_read_endpoint = "https://api.airtable.com/v0/app6bXTYntS36fgSf/Table%202?api_key=keyEKVb9dgJuIAFSg";
        $.getJSON(airtable_read_endpoint, function(result) {
              $.each(result.records, function(key,value) {
                  items = [];
                  items.push(value.fields['Name']);
